...
\* \[프로젝트 정의서](doc/project\_definition.md)
\* \[프로젝트 관리 계획서](doc/Project\_Management\_Plan.md)
\* \[대상 시스템 품질 요소 측정](doc/quality\_factors.md)
...
| 요구사항 ID | 요구사항 명칭 | U\_01 (관리) | U\_02 (공유) | U\_03 (복구) | 비고 |
| :--- | :--- | :---: | :---: | :---: | :--- |
| \*\*FR\_001\*\* | 다단계 폴더 구조를 생성하고 파일을 관리할 수 있다. | O | | | |
| \*\*FR\_002\*\* | 파일명, 날짜, 업로더, 파일 유형 등 다중 조건으로 파일을 검색할 수 있다. | O | | | |
| \*\*FR\_003\*\* | 세분화된 권한 및 만료 기간이 포함된 공유 링크를 생성할 수 있다. | | O | | |
| \*\*FR\_004\*\* | 파일의 이전 버전을 확인하고 복구할 수 있다. | | | O | |
| \*\*NFR\_001\*\* | 200명 동시 접속 시에도 병목 현상이 없어야 한다. | O | O | O | 효율성 |
| \*\*NFR\_002\*\* | 데이터 전송은 암호화되고 RBAC 기반으로 통제되어야 한다. | O | O | O | 보안성 |
| \*\*NFR\_003\*\* | 서버 자원 절약을 위해 변경 이력은 최근 1달간만 보관한다. | | | O | 신뢰성 |
